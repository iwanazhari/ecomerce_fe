...
const loyaltyKeys = {
  ...
  transactions: (params?: LoyaltyListParams) =>
...
const shippingKeys = {
  ...
  calculate: (data: ShippingCalculationInput) =>
    [...shippingKeys.all, "calculate", data] as const,
};
...
export function usePrepareCheckout() {
  return useMutation({
    mutationFn: (data: CreateOrderInput) =>
      checkoutService.prepareCheckout(data),
  });
}
...
/** Create Midtrans Snap token from an existing order */
export function useCreateSnap() {
  return useMutation({
    ...
    }) => paymentService.createSnapFromOrder(orderId, amount, paymentType),
  });
}
...
export function useRemoveFromWishlist() {
  ...
  return useMutation({
    ...
    onMutate: async (productId) => {
      ...
      queryClient.setQueryData(
        wishlistKeys.current(),
        (old: { items: { productId: string }[] } | undefined) => {
          if (!old) return old;
          return {
            ...old,
