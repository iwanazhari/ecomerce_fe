...
import { Upload, X, Image as ImageIcon, Loader2, Star, StarOff } from 'lucide-react'
...
export function ImageUpload({
  ...
  const handleFiles = useCallback(async (files: FileList | File[]) => {
    ...
    try {
      // Upload to Medusa → MinIO
      const result = await adminUploads.uploadFiles(validFiles)
  ...
  const setPrimary = useCallback((imageId: string) => {
        ...
        >
          {uploading ? (
            <>
              ...
              <p className="text-sm text-foreground-muted">Mengupload gambar...</p>
            ...
            <>
              ...
              <p className="text-sm text-foreground-muted">
                Seret gambar ke sini atau <span className="text-primary font-semibold">klik untuk pilih</span>
              </p>
              <p className="text-xs text-foreground-subtle">
                JPG, PNG, WebP — Maks {maxSizeMB}MB per gambar ({images.length}/{maxImages})
              </p>
            </>
          )}
          <input
          ...
          {displayImages.map((img) => (
            <div
              ...
              className={cn(
                ...
                img.isPrimary && 'ring-2 ring-primary ring-offset-[2px] ring-offset-background',
              )}
            >
              {/* Image */}
