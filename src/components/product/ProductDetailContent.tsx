...
export function ProductDetailContent({
  ...
  // Use SSR data initially, then switch to React Query data
  const displayProduct = product ?? initialProduct ?? null;
  const isPageLoading = isLoading && !displayProduct;

  const selectedVariant = displayProduct?.variants?.[0];
  ...
  const isInWishlist =
    isAuthenticated &&
    wishlist?.items?.some((item) => item.productId === displayProduct?.id);

  // Track recently viewed
  useEffect(() => {
    if (displayProduct) {
      addProduct(displayProduct.id);
    }
  }, [displayProduct, addProduct]);
  ...
  if ((error && !initialProduct) || !displayProduct) {
    return (
        ...
        <div>
          ...
          <div className="mt-6 flex gap-2">
            <Button
              ...
              disabled={
                !selectedVariant || (stock !== undefined && stock <= 0)
              }
              onClick={() => {
                if (selectedVariant) {
                  const variantId =
                    selectedVariant.id ||
                    `${displayProduct.id}-${selectedVariant.sku || "default"}`;
                  addToCart.mutate({
                    productId: displayProduct.id,
                    variantId,
                    quantity,
                    productTitle: displayProduct.name,
            ...
            <Button
              ...
              onClick={() => {
                if (displayProduct) {
                  addToWishlist.mutate(displayProduct.id);
                }
              }}
            >
              <Heart
            ...
            <TabsContent value="specs">
              <dl className="divide-y divide-border text-sm">
                ...
                <div className="flex py-2">
                  ...
                  <dd className="w-2/3 text-foreground-muted">
                    {displayProduct.weight ? `${displayProduct.weight} kg` : "-"}
                  </dd>
                </div>
                <div className="flex py-2">
                  <dt className="w-1/3 font-medium text-foreground">
      ...
      {recentlyViewed?.length ? (
        <section className="mt-12 border-t border-border pt-8">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-5">
            {recentlyViewed
              .filter((rv) => (rv as any).product?.id !== displayProduct.id)
              .slice(0, 5)
              .map((rv) => (
