  ...
  try {
    ...
    const existingIndex = storedItems.findIndex(
      (item) =>
        ...
        ((item.variantId as string) === '' && (item.productId as string) === input.productId),
    )
    ...
    if (existingIndex >= 0) {
      ...
      storedItems[existingIndex] = { ...existing, quantity: currentQty + input.quantity }
    } else {
      storedItems.push({
...
export async function updateItem(
  ...
  try {
    const storedItems = getStoredItems()
    const index = storedItems.findIndex((item) => (item.id as string) === itemId)
    ...
    if (input.quantity <= 0) {
      ...
      storedItems[index] = { ...(storedItems[index] as Record<string, unknown>), quantity: input.quantity }
    }
...
export async function removeItem(itemId: string): Promise<ApiResponse<Cart>> {
  try {
    const storedItems = getStoredItems().filter((item) => (item.id as string) !== itemId)
    storeItems(storedItems)
    return { success: true, data: buildCart(storedItems) }
  } catch (error: any) {
    return {
