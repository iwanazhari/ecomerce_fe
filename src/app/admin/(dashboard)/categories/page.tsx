...
export default function AdminCategoriesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (modalMode === "create") {
        ...
        await adminCategories.update(selected.id, {
          name: formName,
        } as any);
      }
      setModalOpen(false);
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    try {
      await adminCategories.delete(deleteId);
            ...
            <tbody className="divide-y divide-border">
              {filtered.length === 0 ? (
                <tr>
                  ...
                  >
                    <Tag className="size-10 mx-auto mb-3 text-foreground-subtle" />
                ...
                filtered.map((cat) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm text-foreground-muted line-clamp-1">
                      {cat.description || "-"}
                    </td>
                    <td className="px-6 py-4">
                      {cat.is_active === false ? (
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <Input
            ...
            helperText="Opsional — otomatis dari nama"
          />
          <div>
