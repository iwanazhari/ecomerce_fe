...
export default function AdminExpeditionsPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await adminExpeditions.list({ limit: 100, q: search || undefined })
    ...
    try {
      ...
      if (modalMode === 'create') {
        await adminExpeditions.create({
          name: formName,
          code: formCode,
          tracking_url_template: formTrackingUrl || undefined,
          is_active: formIsActive,
          description: formDescription || undefined,
          flat_rate: rate,
          is_store_delivery: formIsStoreDelivery,
        })
      } else if (selected) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-foreground">Ekspedisi</h1>
          <p className="text-sm text-foreground-muted">{total} ekspedisi total</p>
        </div>
        <Button onClick={openCreate}><Plus className="size-4" /> Tambah Ekspedisi</Button>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="px-6 py-4 text-left text-xs font-bold text-foreground-muted uppercase tracking-wider shadow-inset-small">Kode</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-foreground-muted uppercase tracking-wider shadow-inset-small">Ongkir</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-foreground-muted uppercase tracking-wider shadow-inset-small">Tipe</th>
            ...
            <tbody className="divide-y divide-border">
              {filtered.length === 0 ? (
                <tr><td colSpan={6} className="p-12 text-center text-foreground-muted"><Truck className="size-10 mx-auto mb-3 text-foreground-subtle" /><p>Belum ada ekspedisi</p></td></tr>
              ) : filtered.map((exp) => (
                <tr key={exp.id} className="hover:bg-primary/5 transition-colors">
                  <td className="px-6 py-4">
                    <p className="font-bold text-foreground">{exp.name}</p>
                    {exp.description && <p className="text-xs text-foreground-muted">{exp.description}</p>}
                  </td>
                  <td className="px-6 py-4 text-sm font-mono text-foreground-muted">{exp.code}</td>
                  <td className="px-6 py-4 text-sm font-bold text-foreground">
                    {exp.flat_rate > 0 ? `Rp ${exp.flat_rate.toLocaleString('id-ID')}` : <span className="text-success">Gratis</span>}
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    {exp.is_active ? (
                      <Badge variant="success"><CheckCircle className="size-3 mr-1" /> Aktif</Badge>
                    ) : (
                      <Badge variant="error"><XCircle className="size-3 mr-1" /> Nonaktif</Badge>
                    )}
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => openEdit(exp)} className="size-10 rounded-2xl shadow-inset-small flex items-center justify-center text-foreground-muted hover:text-primary active:shadow-inset-deep transition-all" aria-label="Edit"><Pencil className="size-4" /></button>
                      <button onClick={() => { setDeleteId(exp.id); setDeleteModalOpen(true) }} className="size-10 rounded-2xl shadow-inset-small flex items-center justify-center text-foreground-muted hover:text-error active:shadow-inset-deep transition-all" aria-label="Hapus"><Trash2 className="size-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {/* Create/Edit Modal */}
      <Modal open={modalOpen} onOpenChange={setModalOpen} title={modalMode === 'create' ? 'Tambah Ekspedisi' : 'Edit Ekspedisi'} size="md">
        <form onSubmit={handleSubmit} className="space-y-5">
          <Input label="Nama Ekspedisi" value={formName} onChange={(e) => setFormName(e.target.value)} required placeholder="JNE" />
          <Input label="Kode" value={formCode} onChange={(e) => setFormCode(e.target.value.toLowerCase())} required placeholder="jne" helperText="Kode unik (huruf kecil, tanpa spasi)" />

          {/* Kurir Toko Toggle */}
          <div className="p-4 rounded-2xl bg-surface shadow-inset-small">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-foreground">Kurir Toko</p>
                <p className="text-xs text-foreground-muted">Aktifkan jika ini adalah pengiriman oleh toko sendiri</p>
              </div>
              <button
                type="button"
              ...
              >
                <div className={`absolute top-0.5 left-0.5 w-6 h-6 rounded-full bg-white shadow-sm transition-transform duration-200 ${formIsStoreDelivery && 'translate-x-5'}`} />
              </button>
            </div>
          </div>
          ...
          {/* Flat Rate Ongkir */}
          <div>
            <label className="text-sm font-semibold text-foreground block mb-2">Ongkir (Rp)</label>
            <input
              type="number"
              min="0"
              step="1000"
            ...
            />
            <p className="text-xs text-foreground-muted mt-1">Masukkan 0 untuk gratis ongkir</p>
          </div>

          <Input label="URL Tracking Template" value={formTrackingUrl} onChange={(e) => setFormTrackingUrl(e.target.value)} placeholder="https://www.jne.co.id/tracking?awb={tracking_number}" helperText="Gunakan {'{tracking_number}'} sebagai placeholder" disabled={formIsStoreDelivery} />
          <div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <div className={`absolute top-0.5 left-0.5 w-6 h-6 rounded-full bg-white shadow-sm transition-transform duration-200 ${formIsActive && 'translate-x-5'}`} />
            </button>
            <span className={`text-sm font-semibold ${formIsActive ? 'text-success' : 'text-foreground-muted'}`}>
      ...
      {/* Delete Modal */}
      <Modal open={deleteModalOpen} onOpenChange={setDeleteModalOpen} title="Hapus Ekspedisi" size="sm">
        <p className="text-foreground-muted mb-6">Apakah Anda yakin ingin menghapus ekspedisi ini?</p>
