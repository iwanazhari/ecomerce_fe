...
const whyChooseFeatures = [
  {
    ...
    icon: Filter,
    features: ['Pre-Filter Sedimen', 'Carbon Block', 'UF Membrane', 'Post-Carbon', 'Mineral Stage'],
  },
  {
    ...
    icon: ShieldCheck,
    features: ['Standar WHO', 'Sertifikasi SNI', 'Uji Lab Terakreditasi', 'Laporan Kualitas'],
  },
  {
    ...
    icon: Leaf,
    features: ['Tanpa Listrik', 'Zero Waste', 'Hemat Biaya', 'Ramah Lingkungan'],
  },
]
...
const howToSteps = [
  {
    ...
    description: 'Pilih produk Waterpro dan checkout dengan mudah melalui website kami.',
  },
  {
    step: '02',
    title: 'Pasang di Keran',
    description: 'Pasang filter di keran dapur Anda dalam hitungan menit tanpa alat khusus.',
  },
  {
    step: '03',
    title: 'Alirkan Air Bersih',
    description: 'Buka keran dan nikmati air bersih langsung dari filter Waterpro.',
  },
  {
    step: '04',
    title: 'Nikmati Setiap Tetes',
    description: 'Air bersih, segar, dan aman untuk minum langsung seluruh keluarga.',
  },
]
    ...
    <div className="group relative rounded-2xl border border-border/60 bg-surface overflow-hidden shadow-soft card-hover perspective-card">
      <div className="flex flex-col sm:flex-row">
        ...
        <div className="flex flex-col justify-between p-5 sm:w-1/2">
          <div>
            <p className="text-xs text-foreground-subtle uppercase tracking-wider">{product.brand || 'Waterpro'}</p>
            ...
            {product.shortDescription && (
              <p className="text-sm text-foreground-muted mt-2 line-clamp-2">{product.shortDescription}</p>
            )}
...
function SmallProductCard({ product }: { product: Product }) {
  ...
  return (
    ...
    >
      ...
      <div className="p-3">
        <p className="text-[10px] text-foreground-subtle truncate">{product.brand || 'Waterpro'}</p>
        <h4 className="text-sm font-semibold text-foreground truncate mt-0.5">{product.name}</h4>
        {variant && (
          <p className="text-sm font-bold text-primary mt-1">{formatCurrency(variant.price)}</p>
        )}
      </div>
    </Link>
  )
...
export function HomePage() {
  const { data: products, isLoading: productsLoading } = useProducts({ sort: 'newest', limit: 8 })
  const { data: categories } = useCategories()
  const [useCaseIndex, setUseCaseIndex] = useState(0)

  const nextUseCase = () => setUseCaseIndex((i) => (i + 1) % useCases.length)
  const prevUseCase = () => setUseCaseIndex((i) => (i - 1 + useCases.length) % useCases.length)
  ...
  return (
    <div className="overflow-hidden">
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <div className="order-1 lg:order-1">
              ...
              <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-4 py-1.5 text-sm mb-6">
                ...
                <span className="text-primary font-semibold">New 2026 Collection</span>
              </div>
              ...
              <p className="mt-6 text-lg text-foreground-muted leading-relaxed max-w-xl">
                Filter air RO 5-tahap yang menghilangkan 99.9% kontaminan. Nikmati air jernih, segar, dan aman langsung dari keran Anda.
              </p>
              ...
              <div className="mt-8 flex flex-wrap gap-8">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-foreground-muted text-sm">(2,560 ulasan)</span>
                </div>
                <div className="flex items-center gap-2">
                  <ShieldCheck className="size-5 text-primary" />
                  <span className="text-foreground font-semibold text-sm">SNI & WHO</span>
                </div>
              </div>
              ...
              <div className="mt-8 flex flex-col sm:flex-row gap-3">
                <Link href={ROUTES.PRODUCTS}>
                  <Button variant="primary" size="lg" className="w-full sm:w-auto shadow-button">
            ...
            <div className="order-2 lg:order-2">
              {featuredProducts.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center rounded-2xl border border-border bg-surface p-12 shadow-soft">
                  ...
                  <p className="text-foreground-muted text-sm">Produk segera hadir</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          ...
          <div className="flex flex-wrap items-center justify-center gap-8 sm:gap-12">
            {[
              ...
              <div key={label} className="flex items-center gap-2 text-foreground-muted">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          ...
          <div className="mb-16">
            <h2 className="text-3xl font-extrabold text-foreground sm:text-4xl tracking-tight">
              Fitur{' '}
              <span className="bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent">Utama</span>
            </h2>
            <p className="mt-4 text-lg text-foreground-muted max-w-xl">
              Filtrasi 5 tahap, bebas bakteri, dan rasa segar — air bersih untuk keluarga Anda.
            </p>
          </div>
            ...
            {featureStats.map((stat) => (
              <div key={stat.label} className="group">
                <div className="flex items-baseline gap-1">
                  ...
                  <span className="text-sm font-bold bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent">{stat.unit}</span>
                </div>
                <h3 className="mt-2 text-sm font-bold text-foreground">{stat.label}</h3>
                <p className="mt-1 text-sm text-foreground-muted">{stat.description}</p>
              </div>
            ))}
          </div>
          ...
          <div className="mt-20 grid gap-12 lg:grid-cols-2 items-center">
            ...
            <div>
              ...
              <div className="mt-6 flex items-center gap-3">
                ...
                />
                <div>
                  <p className="text-sm font-bold text-foreground">{testimonial1.author}</p>
                  <p className="text-xs text-foreground-subtle">{testimonial1.date}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="grid gap-12 lg:grid-cols-2 lg:gap-16 items-center">
              ...
              <div className="flex flex-col justify-center">
                ...
                <div className="mb-4 flex items-center gap-2">
                  ...
                  <span className="text-sm text-foreground-subtle">256 Ulasan</span>
                </div>

                {/* Name & Price */}
                <h2 className="text-2xl font-extrabold text-foreground sm:text-3xl tracking-tight">{featuredProducts[0].name}</h2>
                <div className="mt-3 flex items-baseline gap-3">
                  <span className="text-2xl font-extrabold text-foreground">
                    {formatCurrency(featuredProducts[0].variants?.[0]?.price ?? 0)}
                  </span>
                  {featuredProducts[0].variants?.[0]?.comparePrice && (
                    <span className="text-lg text-foreground-subtle line-through">
                      {formatCurrency(featuredProducts[0].variants[0].comparePrice)}
                    </span>
                  )}
                </div>
                ...
                <p className="mt-6 text-foreground-muted leading-relaxed">
                  {featuredProducts[0].shortDescription || featuredProducts[0].description}
                </p>

                {/* Buy button */}
                <div className="mt-8">
                  <Link href={ROUTES.PRODUCTS}>
                    <Button variant="primary" size="lg" fullWidth className="shadow-button">
                      Beli Sekarang
                    </Button>
                  </Link>
                </div>
      ...
      <section className="bg-background py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          ...
          <div className="mb-16 text-center">
            <h2 className="text-3xl font-extrabold text-foreground sm:text-4xl tracking-tight">
              Mengapa Memilih{' '}
              <span className="bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent">Waterpro Filter?</span>
            </h2>
            <p className="mx-auto mt-4 max-w-xl text-foreground-muted text-lg">
              Dirancang untuk gaya hidup aktif Anda. Performa, durabilitas, dan kenyamanan dalam satu filter.
            </p>
          </div>
            ...
            {whyChooseFeatures.map((feature, index) => {
              ...
              return (
                ...
                >
                  {/* Image */}
                  <div className="relative perspective-card">
                    <div className={`aspect-square overflow-hidden rounded-2xl shadow-soft border border-border/60 transition-transform duration-500 ${index % 2 === 0 ? 'transform rotate-y-[6deg] hover:rotate-y-[2deg]' : 'transform rotate-y-[-6deg] hover:rotate-y-[-2deg]'}`}>
                      <Image
                  ...
                  {/* Content */}
                  <div className="flex flex-col justify-center">
                    <h3 className="text-xl font-extrabold text-foreground tracking-tight">{feature.title}</h3>
                    <p className="mt-4 text-foreground-muted leading-relaxed text-base">{feature.description}</p>
                    <ul className="mt-6 space-y-3">
                      {feature.features.map((f) => (
                        <li key={f} className="flex items-center gap-3 text-sm text-foreground-muted">
      ...
      <section id="reviews" className="bg-surface py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="mx-auto max-w-2xl text-center">
            <h2 className="text-3xl font-extrabold text-foreground sm:text-4xl tracking-tight">
              Apa Kata{' '}
              <span className="bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent">Pelanggan Kami</span>
            </h2>
            <p className="mt-4 text-lg text-foreground-muted">
              Pelanggan kami menyukai Waterpro. Berikut alasannya trusted oleh banyak keluarga Indonesia.
            </p>
          </div>
          ...
          <div className="mt-12 grid gap-6 sm:grid-cols-2">
            {customerReviews.map((review, i) => (
              <div key={i} className="rounded-xl bg-background border border-border/60 p-6 shadow-soft card-hover">
                {/* Stars */}
                <div className="flex items-center gap-1">
                  ...
                  <span className="ml-2 text-xs text-foreground-subtle">{review.date}</span>
                </div>
                <p className="mt-4 text-sm text-foreground leading-relaxed">{review.text}</p>
      ...
      <section className="bg-surface py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="mb-12 max-w-xl">
            ...
            <p className="mt-4 text-lg text-foreground-muted">
              Waterpro dirancang untuk fleksibilitas. Nikmati air bersih berkualitas di setiap sudut rumah Anda.
            </p>
          </div>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              { title: 'Kamar Mandi', icon: ShowerHead, desc: 'Kulit & Rambut Sehat' },
              { title: 'Bayi', icon: Baby, desc: 'Aman untuk MPASI' },
              { title: 'Kantor', icon: Briefcase, desc: 'Hidrasi Produktif' },
            ].map((item) => (
              <div key={item.title} className="group rounded-2xl border border-border/60 bg-background p-6 shadow-soft card-hover text-center">
      ...
      <section className="bg-background py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="max-w-xl">
            <h2 className="text-3xl font-extrabold text-foreground sm:text-4xl tracking-tight">
              Cara Memulai dengan{' '}
              <span className="bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent">Waterpro</span>
            </h2>
            <p className="mt-4 text-lg text-foreground-muted">
              Memasang Waterpro itu mudah dan cepat! Ikuti langkah sederhana ini.
            </p>
          </div>
      ...
      <section className="bg-surface py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="max-w-xl">
            <h2 className="text-3xl font-extrabold text-foreground sm:text-4xl tracking-tight">
              The Water Lab —{' '}
              <span className="bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent">Tips & Panduan</span>
            </h2>
            <p className="mt-4 text-lg text-foreground-muted">
              Jelajahi tips ahli, panduan produk, dan inspirasi untuk mendapatkan manfaat maksimal dari filter Waterpro.
            </p>
          </div>
          ...
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {blogPosts.map((post, i) => (
              ...
              >
                ...
                <p className="mt-1 text-xs text-foreground-subtle">{post.date}</p>
              </div>
            ))}
          </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="text-center">
            <h2 className="text-3xl font-extrabold text-foreground sm:text-4xl tracking-tight">
              ...
              <br />
              <span className="bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent">di Mana Saja!</span>
            </h2>
            <p className="mx-auto mt-4 max-w-xl text-foreground-muted text-lg">
            ...
            <div className="overflow-hidden rounded-2xl">
              <div className="aspect-[16/9] relative border border-border/60 shadow-soft">
                ...
                <div className="absolute bottom-8 left-0 right-0 text-center">
                  <h3 className="text-3xl font-extrabold text-white">{useCases[useCaseIndex].title}</h3>
                  <p className="mt-3 text-white/90 text-lg">{useCases[useCaseIndex].description}</p>
                </div>
              </div>
            </div>
              ...
              {useCases.map((_, i) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all duration-200 ${
                    i === useCaseIndex ? 'w-8 bg-gradient-to-r from-indigo-600 to-violet-600' : 'w-2 bg-border'
                  }`}
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="text-center">
            <h2 className="text-3xl font-extrabold text-foreground sm:text-4xl tracking-tight">
              ...
              <span className="bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent">Passion.</span>
            </h2>
            <p className="mx-auto mt-4 max-w-xl text-foreground-muted text-lg">
              Setiap detail Waterpro dibuat untuk membawa air bersih ke momen sehari-hari Anda.
            </p>
          </div>
          ...
          <div className="mx-auto mt-14 max-w-lg rounded-xl bg-background border border-border/60 p-8 shadow-soft">
            <div className="text-left">
              ...
              <p className="mt-4 text-foreground-muted leading-relaxed">
                Ketika saya memulai Waterpro, saya punya satu tujuan: membuat filter air yang pas ke kehidupan Anda — tanpa kompromi kualitas, gaya, atau durabilitas.
              </p>
              <p className="mt-4 text-foreground-muted leading-relaxed">
                Saya selalu percaya bahwa air bersih adalah hak semua orang. Dengan Waterpro, kami fokus pada apa yang benar-benar penting: filtrasi 5 tahap, tanpa listrik, desain tangguh yang siap mengikuti petualangan Anda.
              </p>
              <p className="mt-4 text-foreground-muted leading-relaxed">
                Setiap fitur dipertimbangkan dengan matang, karena saya percaya air bersih tidak boleh jadi kompromi.
              </p>
              <div className="mt-8 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-foreground-subtle">CEO & Founder</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-20 sm:px-6 sm:py-28 text-center">
          ...
          <p className="mx-auto mt-4 max-w-lg text-indigo-200 text-lg">
            Bergabunglah dengan ribuan keluarga Indonesia yang sudah mempercayai Waterpro.
          </p>
          <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center">
            <Link href={ROUTES.PRODUCTS}>
              <Button size="lg" className="bg-white text-indigo-900 hover:bg-indigo-50 shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all">
                Belanja Sekarang
      ...
      {categories?.length ? (
        <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6">
          <div className="mb-8 flex items-center justify-between">
            <h2 className="text-xl font-extrabold text-foreground tracking-tight">Kategori</h2>
            <Link href={ROUTES.PRODUCTS} className="text-sm font-semibold text-primary hover:text-primary-hover transition-colors group flex items-center gap-1">
              Lihat Semua
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
            {categories.slice(0, 8).map((cat) => (
              <Link
                ...
                className="group rounded-xl border border-border/60 bg-surface p-5 text-center shadow-soft card-hover"
              >
      ...
      <section className="mx-auto max-w-7xl px-4 pb-16 sm:px-6">
        <div className="mb-8 flex items-center justify-between">
          <h2 className="text-xl font-extrabold text-foreground tracking-tight">Produk Terbaru</h2>
          <Link href={ROUTES.PRODUCTS} className="text-sm font-semibold text-primary hover:text-primary-hover transition-colors group flex items-center gap-1">
            Lihat Semua
            <ArrowUpRight className="size-3.5 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
          </Link>
